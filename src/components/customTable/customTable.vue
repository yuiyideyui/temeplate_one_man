        ...
        <el-table-column  :align="item.align":prop="item.prop" :label="item.label" :show-overflow-tooltip="item.showOverflowTooltip"
          ...
          <template #default="scope" v-if="item.customList">
            <template v-for="(custom, index) in item.customList">
              <span v-if="custom.html" :key="index + 'a'" :class="item.boxClass || ''" :style="item.boxStyle" v-html="custom.html ? custom.html(scope.row[item.prop], scope.row,item.prop) : ''
                " @click="
                custom.click
...
// 递归组件，支持 children 和 customList-->支持多级表头
        ...
        const renderColumns = (columns: any[]) => {
            return columns.map((item, index) => {
                ...
                return (
                    ...
                    >
                        {{
                            default: item.customList
                                ? (scope: any) =>
                                      item.customList.map((custom: any, i: number) => {
                                          const val = scope.row[item.prop]
                                          const row = scope.row
                                          const style = item.boxStyle ?? {}
                                          const className = item.boxClass ?? ''
                                          ...
                                          if (custom.html) {
                                              return (
                                                  <span
                                                      ...
                                                      style={style}
                                                      class={className}
                                                      innerHTML={custom.html(val, row, item.prop)}
                                                      onClick={e => {
                                                          if (custom.click) {
                                                              e.stopPropagation()
                                                              custom.click?.(row)
                                          ...
                                          if (custom.hDom) {
                                              const jsxTableDirective =
                                                  resolveDirective('jsx-table')
                                              return withDirectives(
                                                  ...
                                                  />,
                                                  [[jsxTableDirective!, [custom.hDom, val, row]]]
                                              )
                                          }
...
if (props.customFetchData) {
    const res = useQuery({
        ...
        refetchOnWindowFocus: props.customFetchData?.refetchOnWindowFocus ? true : false //  关闭页面聚焦重新请求
    })
